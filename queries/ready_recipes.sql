WITH base AS
     ...
     instructions AS (
         SELECT ri.recipe_id                         AS recipe_id,
                u1.id                                AS user_id,
                CONCAT(replace_instruction(CONCAT(ri.step, ' - ', ri."text" ->> u1.locale)), ' === ',
                       array_to_string(array_agg(
                                               CASE
                                                   WHEN ri2.unit_id IN (2, 4, 10)
                                                       THEN CONCAT(
                                                           CEILING(ri2.value * hrplw.portions_per_recipe / r.servings),
                                                   ...
                                                   WHEN ri2.unit_id IN (3, 13, 16, 19, 22, 23, 41, 42, 43)
                                                       THEN CONCAT(
                                                           ...
                                                           s.display_text ->> u1.locale::TEXT)
                                                   ELSE
                                                       CONCAT(ri2.value * hrplw.portions_per_recipe / r.servings, ' ',
                                                              u2.display_text ->> u1.locale::TEXT, ' ',
                                                              s.display_text ->> u1.locale::TEXT)
                                                   END
         ...
         FROM recipe_instructions ri
                  ...
                  LEFT JOIN recipe_ingredients ri2
                            ON ri.recipe_id = ri2.recipe_id AND ri.step = ri2.recipe_instruction_id
     ...
     ingredients AS
         (
             SELECT hrplw.recipe_id,
                    hrplw.household_owner_user_id AS user_id,
                    hrplw.email,
                    replace_ingredient(CASE
                                           WHEN ri.unit_id IN (2, 4, 10)
                                               THEN CONCAT(
                                                   CEILING(SUM(ri.value * hrplw.portions_per_recipe / r.servings)),
                                                   u2.display_text ->> u1.locale::TEXT, ' ',
                                                   s.display_text ->> u1.locale::TEXT)
     ...
     nutrients AS
         (
             SELECT risq.recipe_id,
                    ...
                    CASE
                        WHEN REPLACE(n.unit, '/100 g', '') = 'mg'
                            THEN SUM(nd.value * risq.ing_quant_grams / 100 / risq.portions) / 1000
                        WHEN REPLACE(n.unit, '/100 g', '') = 'µg'
                            THEN SUM(nd.value * risq.ing_quant_grams / 100 / risq.portions) / 1000000
             ...
             FROM public.recipe_ingredients_standard_quantities risq
                      INNER JOIN (SELECT DISTINCT recipe_id FROM household_recipe_portions_last_week) hrplw
                                 ON hrplw.recipe_id = risq.recipe_id
             ...
             GROUP BY risq.recipe_id,
                      risq.recipe_title,
                      n.full_name_en,
                      n.full_name_de,
                      n.id,
                      n.unit
             ORDER BY risq.recipe_id,
                      n.nutrient_category_en,
                      n.id
         ),
     main AS
         (
             SELECT b.*,
                    ...
                    CASE
                        WHEN b.locale = 'en' THEN 'Saturated fatty acids'
                        ELSE 'Gesättigte Fettsäuren:' END                                                             AS satfat_title,
                    CASE
                        WHEN b.locale = 'en' THEN 'Polyunsaturated fatty acids'
                        ELSE 'Ungesättigte Fettsäuren:' END                                                           AS unsatfat_title,
                    ...
                    CONCAT(ROUND(SUM(nutrient_gram_per_portion) FILTER ( WHERE n.id = 66 )),
                           'g')                                                                                       AS unsatfat_value,
                    CONCAT(ROUND(SUM(nutrient_gram_per_portion) FILTER ( WHERE n.id IN (67, 68) )),
                           'g')                                                                                       AS satfat_value,
                    CONCAT(ROUND(SUM(nutrient_gram_per_portion) FILTER ( WHERE n.id = 69 ) +
                                 SUM(nutrient_gram_per_portion) FILTER ( WHERE n.id = 133 ) * 2),
                           ' kcal')                                                                                   AS energy_value,
                    CONCAT(ROUND(SUM(nutrient_gram_per_portion) FILTER ( WHERE n.id = 70 )),
                           ' kcal')                                                                                   AS bls_energy_incl_fibers_value,
                    CONCAT(ROUND(CAST(SUM(nutrient_gram_per_portion) FILTER ( WHERE n.id = 75 ) AS NUMERIC), 1),
                           'g')                                                                                       AS salt_value,
                    CONCAT(ROUND(SUM(nutrient_gram_per_portion) FILTER ( WHERE n.id = 93 )),
                           'g')                                                                                       AS sugar_value,
                    CONCAT(ROUND(SUM(nutrient_gram_per_portion) FILTER ( WHERE n.id = 133 )),
                           'g')                                                                                       AS fibers_value,
                    CONCAT(ROUND(SUM(nutrient_gram_per_portion) FILTER ( WHERE n.id = 134 )),
                           'g')                                                                                       AS protein_value,
                    CONCAT(ROUND(SUM(nutrient_gram_per_portion) FILTER ( WHERE n.id = 135 )),
                           'g')                                                                                       AS fat_value,
                    CONCAT(ROUND(SUM(nutrient_gram_per_portion) FILTER ( WHERE n.id = 136 )),
                           'g')                                                                                       AS carbs_value,
                    CASE
                        WHEN b.locale = 'en' THEN 'Nutrients (per portion)'
                        ELSE 'Nährwerte (pro Portion)' END                                                            AS nutrition_title
             FROM base b
     ...
     odd AS
         (
             ...
             HAVING COUNT(DISTINCT recipe_id) % 2 = 1
         ),
     core AS
         (
             SELECT main.*,
