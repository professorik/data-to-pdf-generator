...
SELECT DISTINCT ON (u.id, lwplf.box_id) u.id                                                                          AS user_id,
                                        ...
                                        array_agg(DISTINCT
                                                CONCAT(
                                                        CASE
                                                            WHEN u.locale = 'de' AND hl.unit_id IN (4, 10) THEN CONCAT(hl.leftovers, hl.wy_ret_unit_name ->> 'de', ' ',
                                                                                                                       hl.sku_name ->> 'de')
                                                            WHEN u.locale = 'en' AND hl.unit_id IN (4, 10) THEN CONCAT(hl.leftovers, hl.wy_ret_unit_name ->> 'en', ' ',
                                                                                                                       hl.sku_name ->> 'en')
                                                            WHEN u.locale = 'de' THEN CONCAT(hl.leftovers, ' ', hl.wy_ret_unit_name ->> 'de', ' ', hl.sku_name ->> 'de')
                                                            ELSE CONCAT(hl.leftovers, ' ', hl.wy_ret_unit_name ->> 'en', ' ', hl.sku_name ->> 'en')
                                                            END, ' --- ',
...
FROM household_recipe_portions_last_week hrplw
         ...
         LEFT JOIN (SELECT id, is_first_order
                    FROM orders
                    WHERE status = 'charged' AND deleted_at IS NULL AND is_first_order) o2 ON o2.id = ou.order_id
